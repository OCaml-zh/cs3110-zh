...
列表不能轻松实现这些可能性。相反，OCaml 程序员使用【记录体|Record】和【元组|Tuple】。
...
*【记录体|Record】*是其他类型数据的复合，每个数据都有名称。OCaml 记录体类似于 C 语言中的结构体。
以下是一个记录体类型定义 `mon` 的例子，用于宝可梦，复用了[变体](variants)部分的 `ptype` 定义：
```{code-cell} ocaml
...
该类型定义了一个具有三个*【字段|Field】*的记录体，分别命名为 `name`、`hp`（生命值）和 `ptype`。
...
要构建记录体类型的值，我们编写一个记录体表达式，它看起来像这样：
```{code-cell} ocaml
...
要访问记录体并从中获取字段，我们使用你在许多其他语言中会期望的点记法。例如：
```{code-cell} ocaml
...
也可以使用【模式匹配|Pattern Matching】来访问记录体字段：
```{code-cell} ocaml
...
记录体表达式内部 `fi=ei` 的顺序无关紧要。例如，`{f = e1; g = e2}` 与 `{g = e2; f = e1}` 完全等价。
...
记录体类型内部 `fi:ti` 的顺序无关紧要。例如，`{f : t1; g : t2}` 与 `{g:t2;f:t1}` 完全等价。
...
注意，记录体类型必须在使用之前定义。这使得 OCaml 能够进行比记录体类型无需定义即可使用时更好的类型推断。
...
* 如果对于所有 `i` 在 `1..n` 中，都有 `ei : ti`，且如果 `t` 被定义为 `{f1 : t1; ...; fn : tn}`，则 `{f1 = e1; ...; fn = en} : t`。
  注意，记录体表达式中提供的字段集必须是记录体类型定义中的完整字段集（但请参阅下面关于*记录体复制*的内容）。
...
这不会修改旧记录体。相反，它构造一个具有新值的新记录体。
`with` 之后提供的字段集不必是记录体类型定义中的完整字段集。
在新复制的记录体中，任何未作为 `with` 一部分提供的字段都从旧记录体中复制。

<!--
Record copy is syntactic sugar. It's equivalent to writing
-->

记录体复制是【语法糖|Syntactic Sugar】。它等价于编写
...
其中 `gi` 的集合是记录体类型的所有字段集减去 `fi` 的集合。
...
* 如果对于所有 `i` 在 `1..n` 中，都有 `pi` 匹配 `vi` 并产生绑定 $b_i$，
  则记录体模式 `{f1 = p1; ...; fn = pn}` 匹配记录体值 `{f1 = v1; ...; fn = vn; ...}`
  并产生绑定集合 $\bigcup_i b_i$。注意，记录体值可能比记录体模式具有更多字段。
...
作为语法糖，提供了另一种形式的记录体模式：`{f1; ...; fn}`。它被脱糖为 `{f1 = f1; ...; fn = fn}`。
...
与记录体一样，【元组|Tuple】也是其他类型数据的复合。但与命名**分量**不同，
它们通过位置来标识。以下是一些元组的例子：
...
-->
超过大约三个分量后，使用记录体而非元组可能更好，因为程序员很难记住哪个分量应该代表什么信息。
```
...
【变体|Variant】与我们刚刚学到的类型（记录体和元组）之间的最大区别是，
变体类型的值是**一组**可能性中的**一个**，
而元组或记录体类型的值提供**一组**可能性中的**每一个**。
回到我们的例子，类型为 `day` 的值是 `Sun` 或 `Mon` 等中的**一个**。
但类型为 `mon` 的值提供 `string`、`int` 和 `ptype` 的**每一个**。
注意在前面两句话中，「或」与变体类型相关联，「和」与元组和记录体类型相关联。
如果你曾经试图决定是使用变体还是元组或记录体，这是一个很好的线索：
如果你需要一个数据**或**另一个，你需要变体；如果你需要一个数据**和**另一个，你需要元组或记录体。
...
【择一类型|One-of Type】更常被称为【和类型|Sum Type】，
【兼具类型|Each-of Type】被称为【积类型|Product Type】。这些名称来自集合论。
变体类似于[不相交并集][disjun]，因为变体的每个值来自多个底层集合之一（到目前为止每个集合只是一个构造子，因此基数为一）。不相交并集确实有时用求和算子 $\Sigma$ 表示。
元组/记录体类似于[笛卡尔积][cartprod]，因为元组或记录体的每个值包含来自多个底层集合中的每一个的值。笛卡尔积通常用乘积算子 $\times$ 或 $\Pi$ 表示。
